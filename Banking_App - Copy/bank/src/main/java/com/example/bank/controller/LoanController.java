...
public class LoanController {
    ...
    @Autowired
    private com.example.bank.repository.AccountOpeningRepository accountOpeningRepository;
    ...
    public String verifyLoanOtp(@RequestParam String otp, HttpSession session, RedirectAttributes redirectAttributes,
        ...
        if (user != null && user.getAccountNumber() != null) {
            com.example.bank.entity.AccountOpeningEntity account = accountOpeningRepository
    ...
    public org.springframework.http.ResponseEntity<byte[]> getLoanDocument(@PathVariable Long id,
        ...
        switch (type) {
            case "document":
                data = loan.getDocument() != null ? loan.getDocument() : new byte[0];
                break;
            case "aadhar":
                data = loan.getAadharCardPhoto() != null ? loan.getAadharCardPhoto() : new byte[0];
                break;
            case "pan":
                data = loan.getPanCardPhoto() != null ? loan.getPanCardPhoto() : new byte[0];
                break;
            case "photo":
                data = loan.getUserPhoto() != null ? loan.getUserPhoto() : new byte[0];
                break;
            case "signature":
                data = loan.getSignature() != null ? loan.getSignature() : new byte[0];
                break;
        }
    ...
    @GetMapping("/customer/active-loans")
    public String activeLoans(HttpSession session, Model model) {
        Long userId = (Long) session.getAttribute("userId");
        if (userId == null)
            return "redirect:/login_customer";
        User user = userRepository.findById(userId).orElse(null);
        if (user != null) {
            java.util.List<LoanRequest> myLoans = loanRequestRepository.findByCustomerId(userId);
            model.addAttribute("myLoans", myLoans);
            if (user.getAccountNumber() != null) {
                com.example.bank.entity.AccountOpeningEntity app = accountOpeningRepository
                        .findByAccountNumber(user.getAccountNumber());
                model.addAttribute("app", app);
            }
        }
        return "customer-active-loans";
    }

    @GetMapping("/customer/loan-history")
